...
<!--book 리스트-->
<h3 style="font-size: 24px; margin-left:170px;">출판 예정 도서목록 : ${pageResponse.total}개</h3>
...
<div class="nav">
    <ul>
        <c:if test="${pageResponse.showPrev}">
            <li class="nav_prev">
                <a href="<c:url value="/book/books_toBePublished?page=${pageResponse.page-1}&size=${pageResponse.size}"/>">
                    [PREV]
                </a>
            </li>
        </c:if>
        <c:forEach begin="${pageResponse.start}" end="${pageResponse.end}" var="num">
            <li>
                <a href="<c:url value="/book/books_toBePublished?page=${num}&size=${pageResponse.size}"/>"
