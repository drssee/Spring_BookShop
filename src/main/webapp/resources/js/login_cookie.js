$(document).ready(function () {
    ...
    $("#chk1").change(function () { // 체크박스에 변화가 있다면,
        if ($("#chk1").is(":checked")) { // ID 저장하기 체크했을 때,
            setCookie("remember_id", $("#u_id").val(), 7); // 7일 동안 쿠키 보관
    ...
    // 로그인 유지 쿠키
    var remember_user = getCookie("remember_user");

    if (remember_user.length!==0) { // 그 전에 ID를 저장해서 처음 페이지 로딩 시, 입력 칸에 저장된 ID가 표시된 상태라면,
